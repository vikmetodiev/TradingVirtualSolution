...
import { getUnixTime } from "../utils/NumberUtils";
import { NFT_DEPLOY_CHARGE, NFT_META_ROOT } from "./system";

export function getNFTContract(config: INFTProps, baseURI: string) {
  return `
...
contract CointoolNFT is ERC721, Ownable {
    ...
    uint public mintPrice = ${parseEther(config.mintPrice)};
    uint public startTime = ${getUnixTime(config.startTime)};
    uint public endTime = ${getUnixTime(config.endTime)};
    bool public publicMint = ${config.publicMint ? "true" : "false"};
    uint public userMaxMint = ${config.userMaxMint};
    uint public maxSupply = ${config.totalSupply};
    mapping(uint256 => string) private _tokenURIs;

    event MintPriceUpdated(uint price);
    event PublicMintStatusChanged(bool enabled);

    constructor() payable ERC721("${config.name}", "${
    config.symbol
  }") Ownable() {
        require(msg.value == ${parseEther(
          NFT_DEPLOY_CHARGE[config.chainId]
        ).toString()}, "Invalid fund provided.");
    }
    ...
    function updateTokenURI(uint256 tokenId_, string memory tokenURI_) public onlyOwner() {
