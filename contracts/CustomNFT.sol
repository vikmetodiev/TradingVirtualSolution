...
contract CustomNFT is ERC721, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, uint mintPrice_) ERC721(name_, symbol_) {
        mintPrice = mintPrice_;
    }

    function _baseURI() internal override pure returns (string memory) {
        return "https://abc.com/nft/";
    }
    ...
    function mint(uint256 tokenId) public payable withinTime() publicMintable() {
        if (owner() != _msgSender()) {
        ...
        if (maxSupply != 0) {
            require(totalSupply < maxSupply, "Supply Limit exceeded");
        }
